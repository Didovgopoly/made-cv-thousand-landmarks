...
class Cutout(object):
    ...
    def __init__(self, size=8, elem_name='image'):
        self.size = size
        self.elem_name = elem_name
    ...
    def __call__(self, sample):
        ...
        img[y - self.size:y + self.size, x - self.size:x + self.size,:] = 0
...
class RandomBlur(object):
    """Аугментация, которая вырезает кусок заданного размера из картинки
    статья: https://arxiv.org/pdf/1708.04552.pdf"""
    def __init__(self, blur=5, elem_name='image'):
        self.blur = blur
        self.elem_name = elem_name
...
class ThousandLandmarksDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample["image"] = image
        sample["name"] = self.image_names[idx]
        sample["size"] = image.shape[0]
